...
class Cities:
    """Setting objects for pathfile

    Attributes:
        cities(list): location and population data about cities
    """   ''''''
    def __init__(self, filepath):
        '''Reads words from a file and stores them in the words attribute
        Args:
            filepath(str): path to word list
        Side effects:
            Create cities attribute.
        '''
        self.cities = []
        with open(filepath, "r", encoding = "utf-8") as f:
            for line in f:
                name, lat, lon, pop = line.strip().split("\t")
                x = {"name": str(name),"lat": float(lat),"lon": float(lon)
                    ,"pop":int(pop)}
    ...
    def nearest(self, lat, lon, min_population=0, n=10):
        ...
        Args:
            lat (_float_): a latitude in decimal degrees
            lon (_float_): a longitude in deciaml degrees
            min_population (int, optional): a minimum popoulation.Defaults to 0.
            n (int, optional): maximum number of results to return. 
            Defaults to 10.

        Returns:
            cities_copy (list): a list of the n closet city dictionaries from 
            self.cities a population greater than min_population
        """        ''''''
        cities_copy = sorted((c for c in self.cities 
                            if c["pop"] >= min_population), 
                            key = lambda x: 
                            get_dist((lat, lon),(x["lat"],x["lon"]))) 
